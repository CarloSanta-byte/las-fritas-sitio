...
function crearBackend() {
  ...
  class Sheet {
    ...
    getDataRange() {
      return {
        getValues: () =>
          this.rows.map((r) =>
            Array.from({ length: this.getLastColumn() }, (_, i) => r[i] ?? ""),
          ),
      };
    }
    getRange(row, col, numRows = 1, numCols = 1) {
      return {
        ...
        setValues: (values) => {
          values.forEach((r, i) => {
            while (this.rows.length < row + i) this.rows.push([]);
            r.forEach((v, j) => {
              this.rows[row - 1 + i][col - 1 + j] = v;
            });
          });
        },
      };
  ...
  const ss = {
    getSheetByName: (name) => hojas.get(name),
    insertSheet: (name) => {
      const sheet = new Sheet();
      hojas.set(name, sheet);
      return sheet;
    },
  };
  const properties = {};
  ...
  const contexto = vm.createContext({
    ...
    LockService: {
      getScriptLock: () => ({ tryLock: () => !ocupado, releaseLock() {} }),
    },
    PropertiesService: {
    ...
    Utilities: {
      ...
      computeDigest: (alg, value) =>
        crypto.createHash(alg).update(value).digest(),
  ...
  return {
    ...
    ss,
    properties,
    preparar: () => contexto.prepararSistema(),
    post: (body) =>
      contexto.doPost({ postData: { contents: JSON.stringify(body) } }),
    get: (params) => contexto.doGet({ parameter: params }),
    lock: (v) => {
      ocupado = v;
    },
    contexto,
